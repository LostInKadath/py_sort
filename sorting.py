class Sorting(list):
	...
	def Shell(self, reverse=False):		
		def iterator_shell(n):
			'''	Complexity O(n**2); k = [N/2, N/4, N/8, ... , 1] '''
...
#	a = Sorting([5,7,2,6,8,1,3,4])
	...
	print('Bubble({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)
	
	random.shuffle(a)
	t1 = time.perf_counter()
	a.Shell(reverse = direction)
	print('Shell({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)

	random.shuffle(a)
	t1 = time.perf_counter()
	a.Heapsort(reverse = direction)
	print('Heapsort({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)

	random.shuffle(a)
	t1 = time.perf_counter()
	a.Merge(reverse = direction)
	print('Merge({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)
	
	random.shuffle(a)
	t1 = time.perf_counter()
	a.Tree(reverse = direction)
	print('Tree({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)

	random.shuffle(a)
	t1 = time.perf_counter()
	a.Quick(reverse = direction)
	print('Quick({}),\t\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)

	random.shuffle(a)
	t1 = time.perf_counter()
	a.QuickI(reverse = direction)
	print('Quick iterative({}),\ttime = {:.3f} sec'. format(n, time.perf_counter() - t1))
#	print(*a)
